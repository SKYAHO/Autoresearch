...
# DuckDB in-memory 연결은 기본 memory_limit이 시스템 RAM의 ~80%이고
# temp_directory가 없으면 대형 정렬/조인을 디스크로 spill하지 않고 메모리에
# 유지한다. 1.77M행 학습셋 조립의 join+ORDER BY가 여기서 18GB까지 폭발해
# GKE에서 OOM됐다(#271/#291). 모든 연결을 아래 헬퍼로 만들어 작업 메모리를
# memory_limit으로 제한하고 초과분을 temp_directory로 spill시킨다 — 리소스가
# 아니라 코드로 제한된 메모리에 맞춘다. 기본값은 작은 노드(batch-spot 5.88Gi)에
# 들어가도록 보수적으로 잡고, 필요하면 env로 튜닝한다.
DUCKDB_MEMORY_LIMIT = os.environ.get("CTR_TRAINING_DUCKDB_MEMORY_LIMIT", "2GB")
DUCKDB_TEMP_DIR = os.environ.get("CTR_TRAINING_DUCKDB_TEMP_DIR", tempfile.gettempdir())


def connect_duckdb() -> duckdb.DuckDBPyConnection:
    """디스크 spill이 가능하도록 memory_limit/temp_directory를 설정한 DuckDB 연결."""
    con = duckdb.connect()
    con.execute(f"SET memory_limit = '{DUCKDB_MEMORY_LIMIT}'")
    con.execute(f"SET temp_directory = '{DUCKDB_TEMP_DIR.replace(chr(39), chr(39) * 2)}'")
    return con
