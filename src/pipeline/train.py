...
def main(
    ...
    if test_set_output is None:
        test_set_output = os.path.join(project_root, config["artifacts"]["test_set_path"])
    elif not os.path.isabs(test_set_output):
        test_set_output = os.path.join(project_root, test_set_output)
    os.makedirs(os.path.dirname(test_set_output), exist_ok=True)
    test_df.to_csv(test_set_output, index=False)
    print(f"  [저장] Test set (held-out): {test_set_output}")
    ...
    if model_output is None:
        ...
        model_output = os.path.join(project_root, model_output)

    if feature_columns_output is None:
        feature_columns_output = os.path.join(project_root, config["artifacts"]["feature_columns_path"])
    elif not os.path.isabs(feature_columns_output):
        feature_columns_output = os.path.join(project_root, feature_columns_output)

    os.makedirs(os.path.dirname(model_output), exist_ok=True)
    os.makedirs(os.path.dirname(feature_columns_output), exist_ok=True)

    save_model(model.model, model_output)
    save_feature_columns(feature_columns, feature_columns_output)
