...
def refresh_selected_experiment(client: ExperimentClient, state: WorkbenchState) -> bool:
    ...
    try:
        ...
        # Step은 cursor를 쓰지 않고 매번 전체를 다시 읽는다. Step은 PATCH로 갱신되는
        # mutable 리소스인데, cursor는 `after_id` **뒤의** row만 돌려주므로 이미 받은
        # Step의 상태 변화(STARTED -> COMPLETED)를 영원히 관측하지 못한다.
        steps, step_cursor = client.get_steps(state.selected_id, None)
        append_event_page(state, events, event_cursor)
        append_log_page(state, logs, log_cursor)
        append_step_page(state, steps, step_cursor)
