...
- **담당 아님(인접 책임)**: LightGBM→ONNX 변환은 학습측 `autoresearch.model_training.model_utils.convert_lgbm_to_onnx`,
  아티팩트 manifest 검증·로딩은 `applications.reranking_api.model_loader`, 재랭킹·calibration 체이닝은
  `applications.reranking_api.service.Reranker`가 담당한다. 이 어댑터는 predict_proba만 제공한다.
