...
| 3 | **IP 단위 차단** | 403 (suspension 계열) 또는 **모든 Key 가 동일한 비-Key 에러로 실패** | **저 (꼬리 케이스)** — 반복 패턴 감지 시 |
...
ResilientYouTubeClient.make_callables()   ← client.py (신규)
   ...
   ├─④ IP 밴 시그니처(전 키 동일 실패) → Cloud Run 프록시 경로로 전환
...
- 각 자원(videos/channels/videoCategories)별 callable 은 내부적으로:
  ...
  5. IP 밴 시그니처 감지 시 proxy_url 경로(`requests.get`)로 전환
...
| 5xx, `TimeoutError`, `ConnectionError` | 일시적 | backoff 재시도 (같은 Key) |
| 한 collection run 내 **모든 Key 가 동일한 비-Key 에러**로 실패 | **IP 밴 시그니처** | Cloud Run 프록시 경로로 전환 |
...
프록시는 상태를 가지지 않는 단순 전달자다. Key 는 collector 가 query param 으로 붙여 보내며 프록시는 Key 를 다루지 않는다.

```
collector(프록시 모드):
  GET {proxy_url}/youtube/v3/videos?part=...&chart=mostPopular&key=<현재키>
프록시(FastAPI):
  동일 query string 으로 https://www.googleapis.com/youtube/v3/videos?... 전달
...
- **재시도 금지**: 현재 DAG `default_args={"retries": 2}` 로 인해 일시적 실패는 재시도하지만, `CollectionExhausted` 는 터미널(모든 Key·프록시 소진) 상태라 재시도가 무의미하다. 따라서 이 예외는 Airflow 가 재시도하지 않도록 처리해야 한다(예: 태스크 본문에서 `CollectionExhausted` 를 잡아 `context["task_instance"].state = "failed"` 처리 후 `AirflowFailException` 승격, 또는 해당 태스크에만 `retries=0` 오버라이드). 일시적(5xx 등) 실패는 기존대로 `retries` 로 재시도.
...
Key 는 Secret Manager, collector(`client.py`)가 런타임에 로드. Key 가 평문으로 코드/로그에 노출되지 않도록 주의.
