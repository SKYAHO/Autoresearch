...
| `event_timestamp` | TIMESTAMP | `collected_at` 또는 `video_trending_date` | 가능하면 `collected_at` 사용 | 피처 수집/유효 시각 |
...
WITH parsed AS (
  SELECT
    video_id,
    COALESCE(collected_at, video_trending_date, video_published_at) AS event_timestamp,
    ...
    DATE_DIFF(
      DATE(COALESCE(collected_at, video_trending_date, video_published_at)),
