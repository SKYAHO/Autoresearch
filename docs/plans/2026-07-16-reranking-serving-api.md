...
- [ ] **Step 1: 실패하는 테스트 작성 — model_utils 라운드트립**
...
- [ ] **Step 5: 실패하는 테스트 작성 — collect_categorical_categories**
...
- [ ] **Step 1: 실패하는 테스트 작성 — 학습 카테고리 순서로 캐스팅**
...
- [ ] **Step 1: 실패하는 테스트 작성 (동작 고정용 — 현재 구현에서도 통과해야 함)**
...
- [ ] **후속 이슈 제안 (구현 금지, 보고만)**: `src/pipeline/evaluate.py:78-80`이 test set 단독 `astype("category")`를 사용해 학습과 category 코드가 어긋날 수 있음 — `categorical_columns.pkl` 아티팩트를 로드하도록 고치는 별도 이슈를 사용자에게 제안한다.
