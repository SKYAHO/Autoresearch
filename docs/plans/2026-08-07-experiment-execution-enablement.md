...
- [x] **`PairedExperimentResult`를 입력으로 하는 Markdown 렌더러 — 완료 (#620).**
      `src/pipeline/experiment_report_markdown.py` + `render-experiment-report` CLI.
      판정도 실행도 하지 않고 네트워크도 쓰지 않는다
- [x] **포함 항목 — 계약이 담는 것만 렌더링하고 나머지는 "미측정"으로 명시 (#620).**
      요구된 9개 중 4개가 아직 결과 계약에 없어, 추측 필드를 만들지 않고 부재를
      문서에 남기는 쪽을 택했다. 조용히 생략하면 승격 판단자가 **"없음"과 "0"을
      구분하지 못하고**, 특히 기여 분해가 빠진 리포트는 §근거 ③의 "피처 가설 성공"
      오판을 그대로 반복시킨다

      | 항목 | 상태 |
      | --- | --- |
      | before/after 지표·신뢰구간, seed·분할, 판정·사유, 데이터 provenance | 렌더링 |
      | 가설 | `--hypothesis` 인자. 조달은 호출부 책임 |
      | 탐색 공간, 우승 조합 | 미측정 — Stage 3이 만든다 |
      | 기여 분해, 보조 지표 | 미측정 — Stage 4가 만든다 |

- [ ] 리포트를 만드는 **호출부**. 현재 `render-experiment-report`를 부르는 주체가 없고,
      가설 본문을 Experiment API에서 읽어 넘기는 경로도 없다
- [ ] 사람이 승격을 판단할 수 있는 정보가 빠짐없이 담기는지 검토 — Stage 3·4 완료 후에야
      가능하다
