...
flowchart LR
    ...
    subgraph INF["Inference Server (FastAPI)"]
        REQ["Request<br/>user_id + video_ids 200개"]
        PAIR["(user, video) 페어 200개 전개"]
        FBUILD["Feature Build"]
        MODEL["Model — CTR 예측"]
        POST["PostProcess — top-K 24개"]
        RESP["Response<br/>(video_id, CTR, model_id) × 24"]
    end
    ...
    REQ --> PAIR --> FBUILD --> MODEL --> POST --> RESP
    ONLINE -- "Load Online Features" --> FBUILD

    RESP -- "Video Meta + CTR" --> LLMG --> CLICK
    CLICK -- "action 적재" --> ACT
```
...
서버는 `(user_id, video_id)` 페어 200개로 전개한 뒤, Online Store에서 온라인
피처를 조회해 Feature Build를 수행합니다. 조립된 피처 행 예시:

```
(User Id 1, 7일간 유저 시청 시간, ..., Video Id 1, 비디오 좋아요 여부, ...)
(User Id 1, 100시간, Video Id 1, 1)
(User Id 1, 100시간, Video Id 2, 0)
...
(User Id 1, 100시간, Video Id 200, 1)
```

> **참고:** User ID·Video ID는 조인 키일 뿐 예측에 의미가 없으므로, 실제
> 모델 입력에서는 제외합니다.

모델이 페어별 CTR을 예측하고 PostProcess가 top-K(24개)를 선별해 응답합니다:

```
(Video Id 1, 0.5, model_id)
(Video Id 2, 0.5, model_id)
...
(Video Id 24, 0.5, model_id)
```
