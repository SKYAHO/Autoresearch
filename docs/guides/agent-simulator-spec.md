...
Event Log는 두 단계(Phase 1, 2)로 나뉘어 생성된다. 스키마 골격은 공유하되 Phase 2에서 `rank`가 실제 값이 되고 `exposure_type`/`session_id`/`request_id`/`query` 등이 추가된다(즉 Phase 2 스키마는 Phase 1의 상위집합). `source` 값으로 단계를 구분한다.
...
[`docs/archive/specs/2026-07-06-event-log-long-format-design.md`](../archive/specs/2026-07-06-event-log-long-format-design.md)
...
- **유저(슬레이트)별 `click_propensity` 최고 1건이 관련성 커트라인(`click_threshold`)
  이상일 때만, 그 1건만 "클릭"으로 선정해 추가로**:
  - `click` 1행
  - `view` 1행 (`watch_time_sec = round(watch_fraction × duration_sec)`)
  - `like` 1행 — `would_like=true`일 때만 (코드 파생 `derive_would_like`; LLM 출력 아님)
...
유저(슬레이트)당 클릭은 최대 1건이며, 그 유저의 노출 중 `click_propensity` 최고
값이 관련성 커트라인(`click_threshold`, 필수 지정값, #260) 이상일 때만 발생한다(미만이면
그 유저는 클릭 0건). CTR(= click 행 수 / impression 행 수, 로그 전체 집계)은 더 이상 코드가
강제하는 목표 비율이 아니라, 이 커트라인과 LLM `click_propensity` 판단 품질에 따라
**창발(emergent)하는 결과 지표**다.
...
Phase 1과 동일한 방식으로 Persona 관심사와 영상 주제를 매칭해 클릭 여부를 시뮬레이션한다. 유저(슬레이트)별 최고 1건 + 관련성 커트라인(`click_threshold`) 규칙도 동일하게 유지하며, CTR은 창발 지표로 관찰만 한다.
...
| 클릭 선정 | 유저(슬레이트)별 `click_propensity` 최고 1건이 `click_threshold` 이상일 때만 클릭(최대 1건/유저) |
| 라벨 비율(CTR) | click 행 수 / impression 행 수 — 강제 목표 아님, 커트라인·모델 판단에 따라 창발하는 결과 지표 |
