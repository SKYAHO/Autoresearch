...
값을 넣기 전에 **주 지표 하한**을 정해야 한다. 지금 판정 조건은 "지표 기준 미달"이기만
하면 성립하므로, **정체(delta≈0)와 대폭 악화가 구분되지 않는다.** 기한만 지나면 주
지표가 얼마나 나빠졌든 승격 후보가 된다.

시간 기반 재학습의 논리("옛 test set 점수가 낮아도 최신 데이터로 학습한 모델이 지금
트래픽에서는 낫다") 자체는 악화 후보를 올리는 방향을 지지한다. 문제는 **하한이 아예 없는
조합**이 있다는 것이다 — Issue Form에서 guardrail을 `없음`으로 선언한 가설에서는
guardrail 검사가 즉시 통과하므로, 하드 리밋이 켜진 순간 **어떤 하한도 없이** 승격된다.

후보 세 가지와 트레이드오프는 spec §4.4에 있다. **정책값이 비어 있는 동안은 이 경로가
실행되지 않으므로 위험이 잠재적이다** — 값을 넣는 순간 실재하게 된다.

또 하나: **producer가 guardrail 값을 실제로 싣는지 확인**해야 한다. Issue Form이
guardrail을 선언했는데 producer가 값을 안 보내면, 기한 도달 시 사유가
`guardrail_metric_missing`이 되어 **승격되지 않는다.** 하드 리밋이 켜졌는데 아무 일도
일어나지 않는 상태가 될 수 있다.
