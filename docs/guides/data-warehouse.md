...
| --- | --- | --- | --- |
| raw (데이터 레이크 적재) | `{raw_dataset}` | `data_lake_raw` | `data_lake_action_log`, `data_lake_youtube_trending_kr`, `asset_virtual_user_vu_1000` |
...
| 임베딩 중간 산출물 | `{embedding_dataset}` | `autoresearch_dev_analytics` | `user_topic_embedding`, `category_embedding` |

`user_topic_embedding` 과 `category_embedding` 은 Feast Feature View 소스가 아니라
`user_category_similarity` 를 만들기 위한 중간 산출물이므로, Feast 전용 dataset 인
`feast_offline_store` 가 아니라 analytics dataset 에 둡니다. 두 테이블의 존재는
Terraform (`Autoresearch-infra`) 이 보장하고 스키마는 적재 스크립트가 소유합니다
...
GCS raw parquet 을 BigQuery 로 적재하는 `scripts/load_raw_to_bigquery.py` 의
`--dataset` 기본값은 raw 계층(`CTR_TRAINING_BQ_RAW_DATASET`, 미설정 시
`data_lake_raw`)입니다(#303). 인자 없이 실행해도 소비자
(`feature_store_build`, `build_training_dataset`)가 읽는 dataset 과 일치합니다.

`asset_virtual_user_vu_1000`는 `--raw-dataset`에 존재해야 하는 raw source
table이다. GCS 원본 parquet(`asset/virtual_user/vu_1000.parquet`)에서 이 table을
적재하는 작업은 BigQuery 쓰기 작업 승인을 받은 뒤에만 다음 기존 명령으로 실행한다.
...
반면 `user_static_feature` 와 `user_category_similarity` 는 persona 나 카테고리
설명문이 바뀔 때만 갱신되는 **정적 feature** 이므로 일일 DAG 가 아니라
`scripts/build_static_features.py` 로 수동 실행합니다. 이 스크립트는 GCS persona
parquet 을 external table 로 직접 읽으므로 `asset_virtual_user_vu_1000` BigQuery
테이블에 의존하지 않습니다(해당 테이블은 삭제 예정).
...
> [!IMPORTANT]
> 실제 적재는 `scripts/build_static_features.py`(step `user_static_feature`)가
> 수행합니다. 두 가지가 아래 표기와 다릅니다.
>
> 1. `CREATE OR REPLACE TABLE` 이 아니라 `TRUNCATE` + `INSERT INTO` 를 씁니다.
>    대상 테이블 스키마를 Terraform 이 소유하므로, SELECT 결과에서 스키마를
>    다시 유추하는 `CREATE OR REPLACE` / `WRITE_TRUNCATE` 는 REQUIRED·REPEATED
>    제약을 지워버립니다.
> 2. 원본이 `{dataset}.asset_virtual_user_vu_1000` 이 아니라 **GCS persona
>    parquet 을 가리키는 external table** 입니다(그 BigQuery 테이블은 삭제
>    예정이며 자동 적재 주체가 없습니다). parquet 의 3-level LIST 인코딩을
>    `ARRAY<STRING>` 으로 읽으려면 external table 정의에
>    `ParquetOptions.enable_list_inference = true` 가 필요합니다 — 없으면 키워드
>    컬럼이 `STRUCT<list ...>` 로 노출돼 `ARRAY_CONCAT` 이 실패합니다.
>
> `SELECT` 본문의 변환 규칙 자체는 아래와 동일합니다.

```sql
-- 실행은 scripts/build_static_features.py(step user_static_feature)가 담당한다.
-- persona = GCS asset/virtual_user/vu_1000.parquet external table.
TRUNCATE TABLE `{project}.{dataset}.user_static_feature`;

INSERT INTO `{project}.{dataset}.user_static_feature` (
...
FROM persona   -- GCS asset/virtual_user/vu_1000.parquet external table
...
WITH action_log AS (
  ...
  -- 7일 집계는 이 안에 포함된다.
  -- dt=D는 KST D일 하루치 슬라이스다(#295 A안). 30일 히스토리는 BETWEEN 프루닝으로 조립한다.
  SELECT
    user_id,
    video_id,
    event_type,
...
> [!IMPORTANT]
> 실제 적재는 `scripts/build_static_features.py`(step `user_category_similarity`)가
> 수행하며, 아래 SQL이 그 구현과 동일한 계약이다. 문서의 초기 버전은 768차원을
> `UNNEST`로 행으로 펼쳐 내적했으나, 중간 결과가
> `140,934 topic × 15 카테고리 × 768 ≈ 16억 행`까지 부풀어 실측에서 33분 동안
> 출력 stage가 0행에 머물렀다(평균 8.8 slot으로 사실상 병렬화 실패). 배열을
> 그대로 받는 `ML.DISTANCE(..., 'COSINE')`로 교체해 같은 값을 즉시 계산한다.
>
> 대상 테이블 스키마는 Terraform 소유이므로 `CREATE OR REPLACE`가 아니라
> `TRUNCATE` + `INSERT`를 쓴다. 그리고 계산 규칙 6("topic embedding이 없으면
> 0.0")을 실제로 만족시키려면 전체 유저 × 전체 카테고리 grid에 `LEFT JOIN`해야
> 한다 — 임베딩 테이블만 inner join하면 키워드가 없는 유저가 통째로 누락된다.

```sql
-- {embedding_dataset} = autoresearch_dev_analytics (임베딩 중간 산출물)
-- {dataset}           = feast_offline_store (Feast feature 테이블)
DECLARE ute_version STRING DEFAULT 'user_topic_embedding_v1';
DECLARE ce_version STRING DEFAULT 'category_embedding_v1';

TRUNCATE TABLE `{project}.{dataset}.user_category_similarity`;

INSERT INTO `{project}.{dataset}.user_category_similarity` (
...
WITH categories AS (
  ...
  FROM `{project}.{embedding_dataset}.category_embedding`
  WHERE category_id IS NOT NULL
    AND category_embedding IS NOT NULL
    AND embedding_version = ce_version
),
users AS (
  -- grid 기준은 임베딩 테이블이 아니라 전체 유저 테이블이다(규칙 6, 누락 방지).
  SELECT DISTINCT user_id
  FROM `{project}.{dataset}.user_static_feature`
  WHERE user_id IS NOT NULL
),
grid AS (
...
user_topics AS (
  ...
  FROM `{project}.{embedding_dataset}.user_topic_embedding`
  WHERE user_id IS NOT NULL
...
best AS (
  -- ML.DISTANCE는 코사인 "거리"(1 - 유사도)를 반환하므로 1에서 뺀다.
  SELECT
    u.user_id,
    c.category_id,
    u.topic,
