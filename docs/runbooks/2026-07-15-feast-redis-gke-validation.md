...
`cloudbuild.builds.create`, 전용 Cloud Build SA에 대한 `iam.serviceAccountUser`,
BigQuery write, GKE RBAC pod 생성·exec 권한, 그리고 control plane master authorized
networks에 등재된 네트워크)가 실행한다.

> 인프라 저장소 `Autoresearch-infra#266`/`#269`에서 팀원 계정에 위 권한이 부여됐다
> (`autoresearch` namespace `view`+`port-forward`+`exec`, Cloud Build 실행, 전용
> build SA 사용). 빌드는 아래 §1처럼 **전용 SA를 지정해서** 제출한다.
...
# Cloud Build 전용 실행 SA (Autoresearch-infra #269). 기본 compute SA 대신 이 SA로 빌드한다.
export BUILD_SA="projects/${PROJECT_ID}/serviceAccounts/autoresearch-cloud-build@${PROJECT_ID}.iam.gserviceaccount.com"
...
로컬 docker 없이 Cloud Build로 `Dockerfile.feast`를 빌드한다. 빌드는 **전용 build
SA**(`${BUILD_SA}`)로 실행한다 — 기본 compute SA는 프로젝트의 모든 빌드가 공유하는
주체라, 그 SA로 빌드하면 push 권한이 필요 이상으로 넓게 열리기 때문이다
(`Autoresearch-infra#269`).

사용자 지정 SA로 실행하는 빌드는 로그 대상을 명시해야 하므로 config에
`options.logging: CLOUD_LOGGING_ONLY`를 넣는다. 저장소 루트에서:

```bash
gcloud builds submit \
  --service-account="${BUILD_SA}" \
  --substitutions=_VCS_REF="$(git rev-parse HEAD)",_IMAGE="${IMAGE}" \
  --config=- <<'YAML' .
steps:
...
빌드 성공 시 이미지가 Artifact Registry에 push된다. 빌드 로그는 Cloud Logging에서
확인한다(`gcloud beta builds submit`으로 제출하면 스트리밍도 된다).

> `--service-account` 없이 제출하면 기본 compute SA로 실행된다. 그 경로는
> `Autoresearch-infra#269` 2단계에서 push 권한이 회수될 예정이므로 사용하지 않는다.
