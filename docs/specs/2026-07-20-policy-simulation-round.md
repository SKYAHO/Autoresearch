...
4. **합동 유저별 최고 1건 + 커트라인 판정** — 두 정책의 `ImpressionDraft`를 유저
   단위로 합쳐 `select_clicks_per_slate()`를 1회만 적용한다. 유저(슬레이트)별로
   두 정책 노출을 통틀어 `click_propensity` 최고 1건만 커트라인(`click_threshold`)
   후보로 삼으므로, 어느 정책이 그 자리를 차지했는지 자체가 비교 신호가 된다.
   정책별로 따로 판정하면 유저당 클릭이 최대 2건(정책마다 1건)이 되어 "같은
   유저를 누가 더 잘 설득했는가"라는 비교 취지가 흐려진다. 합동 판정은 클릭이
   `click_propensity` 높은 정책의 노출로 쏠리게 하여 정책 간 상대 비교 신호를
   보존한다. CTR은 더 이상 강제 목표가 아니라 커트라인과 두 정책의 판단 품질에
   따라 창발하는 결과 지표다.
...
| `click_threshold` | 0.55 | 유저별 최고 `click_propensity`가 이 값 이상이어야 클릭(합동 pool 기준 1회, 유저당 최대 1건) |
...
| 지표 | 의미 |
| --- | --- |
| 정책별 CTR | 합동 커트라인 판정 후 클릭/노출 — 강제 목표가 아니라 창발하는 헤드라인 지표 |
| 정책별 평균 `click_propensity` | 커트라인 판정 전 raw 신호 — 판정 방식에 독립적인 보조 지표 |
...
3. 합동 커트라인 판정 비교 테스트: 가짜 draft로 propensity 높은 정책이 클릭을 더
   가져가는지, 유저당 클릭이 최대 1건인지 검증.
