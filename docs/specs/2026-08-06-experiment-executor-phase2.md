...
- Secret은 `auth.json` key 하나를 같은 이름의 파일로 제공하고 mode `0440`을 사용한다.
- `codex-worker`만 `/var/lib/codex`에 read-only로 mount한다. 나머지 일곱 컨테이너에는
  volumeMount 자체를 만들지 않는다.
...
- Secret이 없거나 `auth.json` key가 없으면 Pod 시작을 fail-closed로 중단한다.
- Secret 교체는 새 Experiment Pod부터 적용한다. 실행 중인 Pod에 인증 교체를 강제하지
  않는다.
