...
**모든 갱신 UPDATE에 `WHERE status NOT IN ('COMPLETED', 'FAILED')` 조건을 건다.** 매치된
row가 0건이면 그 사이 다른 요청이 터미널을 확정한 것이므로, 현재 row를 다시 읽어 위 터미널
가드를 그대로 적용한다 — 동일 payload면 `200`, 다르면 `409`다.

조건을 "터미널로 전이할 때만" 거는 좁은 형태로는 부족하다. 구현 중 실측으로 확인한
두 번째 누수 경로가 있다.

- 세션 factory가 `expire_on_commit=False`이므로(`database.py`), 갱신 직전의 SELECT가
  identity map에 남은 **stale 객체**를 돌려줄 수 있다
- stale 값이 비터미널이면, 요청도 비터미널일 때 조건 없는 UPDATE가 나가 **이미 확정된
  Step을 조용히 덮는다**
- `tests/test_experiment_postgres.py::test_step_update_rereads_state_committed_by_another_session`이
  이 경로를 고정한다. 좁은 형태에서 이 테스트는 `DID NOT RAISE`로 실패한다

조건을 모든 갱신에 걸어도 **"비터미널은 자유 전이"는 그대로다.** 이 조건은 저장된 row가
이미 터미널일 때만 매치에 실패하므로, 비터미널 사이의 동시 갱신은 양쪽 다 통과한다 —
정당한 진행 갱신이 튕기지 않는다.

판정 근거는 반드시 **새 SELECT**여야 한다. `session.refresh()`는 항상 SQL을 발행하므로
방금 다른 트랜잭션이 커밋한 값을 본다. 세션 캐시로 판정하면 위와 같은 조용한 덮어쓰기가
그대로 남는다.
...
무한 요청을 막기 위해 한 갱신의 페이지 수에 예산(`STEP_PAGE_BUDGET = 20`, 페이지당
`STEP_PAGE_SIZE = 100`)을 둔다.

**예산에 걸리면 화면에 반드시 드러낸다.** 조용히 나머지를 버리면 상한값만 2000으로 커진 채
"표시 창이 오래된 구간에 고정된다"는 같은 문제가 그대로 남는다.

- `ExperimentClient.get_steps()`는 `(steps, truncated)`를 반환하고, `truncated`는 예산에
  걸려 뒷부분을 읽지 못했음을 뜻한다
- 워크벤치는 `truncated`일 때 "진행 단계" 탭 상단에 경고를 띄운다 — 몇 개까지 읽었는지와
  **최신 단계가 없을 수 있다**는 사실을 함께 알린다
- 이 경고가 뜨는 실험이 실제로 나오면 `updated_after` 계열 필터를 도입한다(v0 범위 밖)
...
- 다른 세션이 터미널을 확정한 뒤에는, 캐시된 비터미널 상태를 들고 있는 세션의 갱신도 `409`다
