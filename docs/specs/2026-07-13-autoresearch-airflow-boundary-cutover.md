...
4. 수동 production-path run 또는 다음 scheduled run을 관찰한다.
5. stdout의 progress event와 마지막 `job_summary`, 모든 task exit code를 확인한다.
6. 최종 partition과 quality summary를 확인한다.

**종료 조건**: 최소 한 번의 production scheduled run이 새 digest와 public CLI로
성공하고 이전 digest가 rollback 후보로 보존돼 있다.

### Phase 5. 레거시 제거

Production 관찰이 끝난 뒤 별도 cleanup PR로만 삭제한다.
...
6. **Autoresearch-airflow — production digest 승격**
   - Helm value, scheduled run 관찰과 rollback evidence
7. **Autoresearch-airflow — wrapper/build cleanup**
   - wrapper, batch Dockerfile/workflow, legacy tests와 문서 제거
8. **Autoresearch — Astro/DAG cleanup**
   - legacy DAG/settings/Dockerfile/test와 stale 문서 제거
9. **Workspace — migration AS-IS/TO-BE 시각화**
...
- [ ] `Autoresearch` image만으로 YouTube collect, action-log shard/merge와 quality
      command를 실행할 수 있다.
- [ ] Application image는 `Autoresearch` checkout 하나로 build·release된다.
- [ ] Airflow 저장소는 application source를 clone하거나 application image를
      build하지 않는다.
- [ ] Airflow DAG와 helper는 같은 git-sync revision으로 배포된다.
- [ ] Airflow source는 `autoresearch.*` 내부 Python API를 import하지 않는다.
- [ ] 모든 KPO는 공개 module command와 지원되는 v1 인자만 사용한다.
- [ ] Production은 application image를 immutable digest로 선택한다.
- [ ] Airflow는 GCS final artifact 삭제, schema 판정과 quality 계산을 하지 않는다.
- [ ] Application stdout은 JSON Lines와 final `job_summary` 계약을 지킨다.
- [ ] 5-shard QA가 collect → shard → merge → quality 전체 경로를 통과한다.
- [ ] 최소 한 번의 production scheduled run이 새 경계로 성공한다.
- [ ] 이전 application digest와 Airflow revision으로 각각 독립 rollback할 수 있다.
- [ ] `Autoresearch`의 레거시 Airflow 표면과 `Autoresearch-airflow`의 wrapper/domain
      logic/중복 batch build가 제거됐다.
- [ ] 두 저장소의 README, agent guide와 운영 runbook이 최종 소유권과 일치한다.
...
| cleanup이 cutover보다 먼저 merge됨 | production scheduled run 관찰을 cleanup entry gate로 강제 |
