...
**입력 방어(필수)**: `w=0`이면 `(1-q)/w`가 0으로 나누기가 된다. "0<w≤1"은 문서상
전제일 뿐이므로, `apply_downsampling_calibration` **함수 진입부에서 범위를 검증**하고
어기면 `ValueError`를 던진다(`assert`가 아니라 명시적 예외 — 최적화 빌드에서 죽지
않도록). 완료조건과 테스트(`w=0`/`w>1`/`w≤0` → `ValueError`)로 고정한다. `w=1.0`은
항등(보정 없음)으로 정상 처리하며 하위호환·downsampling 미사용 경로의 no-op이다.
...
**학습 루프(early stopping) — 현재 무효, 전방 가드로 명시**: 현재 `train.py`/
`lgbm_model.py`에는 early stopping이 **없다**(`fit()`에 `eval_set`/`callbacks`/
`early_stopping_rounds` 없이 고정 `n_estimators=200`으로 학습;
`lgbm_model.py:60`의 `metric="auc"`는 로깅용이지 stopping 기준이 아니다). 따라서
"train만 다운샘플된 uncalibrated q로 val 지표를 재 early stopping이 어긋난다"는
문제는 **지금은 발생하지 않는다**. 다만 이후 early stopping을 도입하고 그 기준을
**proper scoring(logloss/Brier)**으로 잡으면, val 예측에 **보정을 먼저 적용한 뒤**
그 지표로 멈춰야 한다(그렇지 않으면 원분포 아닌 다운샘플 분포 기준으로 멈춰
비최적 iteration에서 종료). AUC/PR-AUC 기준 early stopping이면 결정 5대로 불변이라
무관하다. 즉 **early stopping을 추가할 때 결정 4의 보정 적용 범위를 학습 루프
내부까지 넓혀야 한다** — 이 스펙 범위의 구현에서 early stopping을 새로 켜지는
않지만, 켤 경우의 계약을 여기 못박아 나중에 조용히 깨지지 않게 한다.
...
- **유저 단위 랭킹 지표(Recall@K·NDCG@K)도 동일하게 불변**이다. 한 유저의 모든
  후보에 **같은 monotonic 함수**를 적용하므로 유저 내부 순위가 안 바뀐다.
  "NDCG는 왜 그대로냐"는 질문이 나오지 않도록 함께 명시한다.
...
- **다른 밸런싱 옵션도 감사**: LightGBM은 `scale_pos_weight` 외에
  `is_unbalance=True`라는 또 다른 자동 클래스 밸런싱 옵션이 있다. 실측 결과 현재
  `config.yaml`·`lgbm_model.py` 어디에도 `is_unbalance`가 없다(비활성). downsampling과
  병행하면 같은 이중 보정이 되므로, 가드를 `scale_pos_weight`만이 아니라 **활성
  자동 밸런싱 옵션 일반**으로 확장하고, "다른 밸런싱 옵션이 비활성임"을 완료조건에
  넣어 나중에 누가 켜는 것도 막는다.
...
- `sampling_rate`는 **fit이 필요 없는 상수 1개**다(별도 학습 아티팩트 아님).
  ...
  실제 비율**을 기록한다.
- **기록처 — run param + 모델 버전 tag 둘 다**: MLflow run param과
  registered model version tag는 별개 개체다. run param만 남기면 서빙이
  "모델 버전 → 연결된 run_id → 그 run의 param"이라는 **간접 조회**를 해야 한다.
  그래서 `mlflow.register_model` 직후 `MlflowClient.set_model_version_tag(name,
  version, "sampling_rate", value)`로 **모델 버전 tag에도 함께** 기록한다. 서빙은
  alias로 모델 버전을 로드하는 순간 그 버전의 tag에서 `sampling_rate`를 **직접**
  읽어 간접 조회가 필요 없다(아래 캐싱과 맞물려 서빙 코드가 단순해진다).
  run param은 실험 추적·비교용으로 유지한다.
- **서빙 캐싱 — 로드 시 1회, 요청 중 캐시 값만 사용**: `sampling_rate`는
  **서버 기동(모델 로드) 시점에 한 번만** 읽어 메모리에 캐싱하고, 요청 처리
  중에는 이 캐싱된 값만 쓴다. 서빙은 유저 1명당 후보 수백 개를 한 번에
  스코어링하므로, 요청마다 MLflow tracking server를 호출하면 요청당 네트워크
  왕복이 하나 더 붙어 온라인 지연에 영향을 준다 — 이를 원천 차단한다. 보정 자체는
  벡터 연산 한 줄(`Div/Sub/Add`)이라 후보 배치 전체에 캐싱된 상수로 적용된다.
...
- [ ] `apply_downsampling_calibration`이 `0<w≤1` 범위를 검증하고 위반 시
      `ValueError`(테스트: `w=0`→`ValueError`, `w>1`/`w≤0`도)
- [ ] `sampling_rate < 1.0`이면 `scale_pos_weight`가 1로 강제되고, 아니면
      학습이 fail-closed (회귀 테스트 포함)
- [ ] 다른 자동 밸런싱 옵션(`is_unbalance` 등)이 비활성임을 확인(현재 없음),
      가드가 활성 밸런싱 옵션 일반을 커버
- [ ] downsampling은 train split에만 적용, val/test 원분포 유지(테스트로 고정)
- [ ] `evaluate.py`가 보정 확률로 LogLoss/calibration 지표를 산출, 보정 전/후
      비교가 문서화됨(AUC·PR-AUC·Recall@K·NDCG@K는 불변임을 명시)
- [ ] `sampling_rate` 실현값이 MLflow run param **및 모델 버전 tag**에 기록됨
      (서빙이 alias 로드 시 tag에서 직접 조회)
- [ ] 서빙이 `sampling_rate`를 **로드 시 1회 읽어 캐싱**, 요청 처리 중엔 캐시
      값만 사용(요청당 MLflow 호출 없음)
