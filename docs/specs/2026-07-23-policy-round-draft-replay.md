...
사이드카 스키마를 늘리지 않고 draft 자체에서 판정 유저 집합을 얻는다.
...
## 알려진 한계 — `chunk_size > 0`의 부분 quarantine

리플레이의 커버리지 규칙은 "draft가 하나도 없는 유저 = 원본 라운드의
quarantine(관용), 일부만 있는 유저 = 노출 집합 불일치(실패)"라는 이분법에
기댄다. 이 이분법은 `chunk_size = 0`(이 배치의 기본값)에서만 참이다.

`_generate_drafts_isolated`의 격리 단위는 유저가 아니라 **(유저 × 후보 청크)**
이므로, `chunk_size > 0`으로 실행하면 같은 유저의 청크 중 일부만 격리되고
나머지는 성공할 수 있다. `_raise_if_quarantine_exceeds`는 전체 격리 **비율**만
검사하므로 이런 라운드도 정상 완료되어 draft parquet이 저장된다. 그 라운드를
같은 노출 인자로 리플레이하면 해당 유저가 "부분 커버리지"로 판정되어 실패한다
— 이 spec이 없애려 한 "quarantine 라운드 리플레이 차단"이 그 조건에서 재발한다.

`exposure_args`에 `chunk_size`가 없으므로 노출 인자 검증도 이를 잡지 못한다.
현재 테스트는 모두 기본값(`chunk_size = 0`)으로 실행되어 이 경로를 덮지 않는다.

해소하려면 커버리지 판정을 "원본 라운드에서 실제로 판정된 노출 집합"과 비교하는
방식으로 바꿔야 한다(예: 사이드카 메타에 판정된 `(user, video)` 키 집합이나
그 요약을 기록). 이 spec의 범위 밖이며 별도 이슈로 다룬다.
