...
| 지표 | 현재 | 목표 |
| --- | --- | --- |
| 입력 tok/호출 (24후보) | ~4,100 | ~2,600 (약 35% ↓) |
| 출력 tok/호출 (24후보) | ~800 | ~200 (약 75% ↓) |
...
- **방안 5 — 위치기반 배열 응답**: 출력을 후보 순서대로의 숫자 배열로 받아
  video_id·키 반복을 제거한다.
...
객체로 감싼다. 각 원소는 후보 순서대로 `[click_propensity, watch_fraction]`.
...
video_id 매핑(`jmap`) 대신 **인덱스 정렬**로 파싱한다.

- `data["j"]`(또는 확정 키)를 읽어 후보 리스트와 위치로 정렬한다.
- 각 후보 `i`: `entry = j[i]` → `click_propensity=_clamp01(entry[0])`,
  `watch_fraction=_clamp01(entry[1])`, `would_like=derive(...)`.
- **길이 불일치 처리 (동작 계약 변경)**: 위치 정렬은 길이가 어긋나면 라벨이
  조용히 오정렬된다. 따라서 `len(j) != len(candidates)`이면 부분 보정 대신
  **`schema_fail`로 격리(quarantine)** 한다. (현행은 누락 후보를 비클릭으로
  패딩했으나, 인덱스 방식에서는 패딩이 오정렬을 은폐하므로 금지한다.)
- 타입/구조 오류(entry가 길이 2 배열이 아님 등) → `schema_fail`.
...
1. 신규 프롬프트는 후보를 배열 위치로 식별하고, 응답도 동일 순서 배열로 받는다.
2. 응답 배열 길이 ≠ 후보 수이면 해당 청크를 `schema_fail`로 격리한다.
...
- **위치 오정렬**: 모델이 순서를 어기거나 개수를 틀리면 라벨이 오염된다.
  → 엄격한 길이 검증 + 격리로 방어(계약 2). 프롬프트에 "정확히 N개를 후보
  순서대로" 명시.
