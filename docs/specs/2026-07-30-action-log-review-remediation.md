...
### 갱신 (2026-08-04, #515) — 객체 저장소 publish는 staging을 경유하지 않는다

위 "atomic move" 요구의 **구현 방식**이 바뀌었다. 요구 자체는 그대로다.

기존에는 GCS에 rename이 없어 `staging 객체 생성 → copy → delete`로 흉내냈다. 그런데
삭제 권한이 없으면 임시 객체가 파티션에 남고, 적재가 `dt=<날짜>/`를 와일드카드로 읽어
**같은 내용을 두 번 적재**한다(`event_id` 전역 고유 계약 위반 → 폐루프 정지). 삭제 실패는
경고로만 남아 4일간 드러나지 않았다. 실제 사고 기록은 #515.

**변경:** `filesystem`이 주어진 경로(객체 저장소)에서는 최종 경로에 **직접 쓴다.**

- GCS 객체는 업로드가 finalize되어야 보이므로 부분 파일이 노출되지 않는다.
- 덮어쓰기가 원자적이므로 쓰기 도중 실패하면 기존 객체가 그대로 남는다 —
  "last-known-good 보존" 요구를 staging 없이 만족한다.
- **삭제 권한 의존이 사라진다.** 같은 사고가 권한 문제로 재발하지 않는다.

로컬 경로(`filesystem is None`)는 POSIX에 rename이 있으므로 기존 `copy` + `replace`를
유지한다. 두 분기의 구현이 다른 것은 의도된 것이다.

**검증 한계:** "업로드 완료 전에는 객체가 보이지 않는다"는 GCS의 문서화된 보장이며
테스트 스위트가 증명할 수 있는 성질이 아니다. 단위 테스트는 "최종 경로에 직접 쓴다",
"삭제를 호출하지 않는다", "쓰기 실패 시 기존 객체가 남는다"까지만 단언한다.
