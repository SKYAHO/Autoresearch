...
### 4.4 guardrail은 하드 리밋으로 우회되지 않는다
...
**선택지 두 가지 — 이 spec은 확정하지 않는다:**

- **안 A(근사)**: 버전 `creation_timestamp`를 쓴다. 추가 구현 없음. 위 오차를 감수한다.
- **안 B(정확)**: alias 부여 시각을 기록하는 경로를 만든다. `set_model_alias` 호출부
  또는 registry 태그에 남긴다. **다른 사람 영역(`src/tracking/`)을 건드린다.**

**plan 착수 전 `#461` 소유자 확인이 필요하다**(§8).
...
**다만 3번은 여전히 열려 있다.** 1·2번은 예/아니오라 "승인 완료"로 답이 되지만, 3번은
**두 안 중 선택**이라 그 코멘트로 갈리지 않는다. 그리고 3번이 `src/tracking/registry.py`
(다른 사람 영역)를 건드릴지 말지를 정한다.

따라서 이 spec은 **Proposed를 유지**하고, plan은 §6을 **안 A/안 B 분기**로 구성한다.
분기 지점 전까지의 Task(§9의 1~3, 순수 함수 변경)는 어느 안이든 동일하므로 선행 가능하다.

함께 물어야 할 것(§6의 성립 조건): **champion alias 부여가 버전 생성과 거의 동시였는지,
시차가 있었는지.** 여러 버전을 미리 등록해두고 나중에 alias만 옮기는 운영이면 그 간격이
며칠씩 벌어지고, 그러면 안 A의 근사치는 "며칠 일찍"이 아니라 **거의 항상 도달 상태**가
되어 하드 리밋이 무의미해진다. 이 이력은 `#461` 소유자가 안다.

> 구두 확인은 근거로 적지 않는다(`#485` §7.1). 위 링크는 GitHub에 남은 기록이라 그
> 조건을 만족하지만, **무엇이 승인됐는지**까지는 담고 있지 않다 — 3번 답이 오면 그
> 링크를 여기에 추가한다.
