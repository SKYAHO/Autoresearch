...
### 1번의 조회는 반드시 트랜잭션을 닫고 나온다

**맨 SELECT는 session의 implicit transaction을 열어둔 채 반환한다.** 그 상태로 2·4번의
서비스 함수를 부르면 그 안의 `with session.begin()`이
`InvalidRequestError: A transaction is already begun`으로 죽는다. 결과는 Step이 한 줄도
안 남는 데서 끝나지 않는다 — 실패 후에도 트랜잭션이 남아 **같은 tick 뒤이은 Job의
`create_experiment_log`까지** 같은 예외로 죽는다. 결정 4가 막으려던 상호 차단이 한 층
아래에서 그대로 열린다.

닫는 방법은 조회 뒤 `session.rollback()`이다. `with session.begin()`으로 감싸는 방법도
있지만 그쪽은 **이미 열린** 트랜잭션이 있으면 그 자체로 같은 예외를 내므로 "세션이
깨끗하다"는 가정을 새로 만든다. `rollback()`은 열려 있으면 닫고 없으면 no-op이라 가정이
필요 없고, `create_experiment_log`가 복구 경로 SELECT 뒤에 쓰는 것과 같은 관용구다
(`service.py:636-648`). 읽기만 했으므로 되돌릴 것도 없다.

이 성질은 **실제 `Session`을 통과시켜야만 드러난다.** 조회·생성·갱신을 더블로 주입한
단위 테스트에서는 더블이 `session.begin()`을 부르지 않아 전부 통과한다(#691 리뷰).
...
- **실제 `Session`: 생성·무변화·갱신 3분기를 sqlite fixture로 통과시키고, 각 호출 뒤
  트랜잭션이 남지 않는지 확인한다.** 더블만 쓰면 트랜잭션 경계가 검증되지 않는다.
- **실제 `Session`: 로그 → Step → 로그 순으로 같은 세션에 써서 Step 쓰기가 뒤이은 로그
  적재를 막지 않는지 확인한다.**
- 단위: `sink.write`의 일반 예외가 그 tick **뒤이은 Job**의 로그 수집을 막지 않는다.
