...
종료된 Job도 TTL 동안 목록에 남는데 이는 **필요한 성질**이다 — 완료 직후 마지막 부분
청크를 flush하려면 종료 여부를 알아야 하고, 그 판정을 Job·Pod 상태에서 얻는다.

**그 창은 기본 30초다.** `ORCH_TTL_AFTER_FINISHED_SEC`의 기본값이 30이고
(`launcher/config.py`, `.env.example`, `README.md` 모두 30), 지금 클러스터에 주입된
3600은 `#579`가 `#582` 실패 원인을 모으려고 넣은 **회수 예정 임시값**이다
(`Autoresearch-infra` `launcher-cronjob.yaml`: *"EVALUATING 완주 증거 수집 뒤 30으로
회수한다"*). 계약이 3600에 기대면 안 된다.

여기서 지켜야 할 관계는 **`ORCH_LOG_COLLECT_INTERVAL_SEC` < `ORCH_TTL_AFTER_FINISHED_SEC`**
이다. 기본값(5 < 30)이면 마지막 컨테이너가 종료된 뒤 꼬리 청크를 flush할 기회가 약
6번 있다. 주기를 30 이상으로 올리면 그 기회가 0이 될 수 있고, 마지막 컨테이너의
꼬리 — 실패했다면 실패 원인이 찍히는 자리 — 가 화면에 영영 안 나온다.

**이 관계는 코드로 강제하지 않는다.** 두 값이 서로 다른 워크로드(launcher CronJob과
수집기 Deployment)에 주입되고 소유도 `Autoresearch-infra`에 있어, 한쪽 프로세스가
다른 쪽 설정을 읽어 검증할 수 없다. 대신 여기 제약으로 적고 infra 매니페스트가
둘을 함께 바꾸게 한다. 알려진 한계로 남긴다.
...
| 1개 | 정상 | 수집 |
| 2개 이상 | `backoffLimit=1` 재시도로 발생 가능 (`jobs.py`) | `creationTimestamp` 오름차순으로 **전부** 수집 |

**최신 하나만 고르지 않는다.** 실패한 시도의 마지막 청크가 실패 원인이 찍히는
자리인데, 최신 Pod만 걷으면 화면에서 정확히 그 구간이 사라진다 — #559가 없애려던
상황이 그대로 남는다. 아래 멱등키에 `pod_name`이 들어가 두 시도가 섞이지 않으므로
둘 다 걷어도 안전하다. 비용은 Pod 수만큼의 API 호출인데 `backoffLimit=1`이라 상한이
2다.

오래된 것부터 적재해 워크벤치의 시간순 읽기와 순서를 맞춘다.
...
**읽기 오프셋 캐시는 실측 후에 넣는다.** 근거 없이 상태 관리를 늘리지 않는다.
다음 실행 때 **컨테이너별 로그 크기와 실제 소요 시간을 함께** 측정해 이 전제를 다시 본다
— 컨테이너 통합으로 Codex 호출이 2회가 되면 늘어난다.

### 쓰기는 증폭을 두지 않는다 — 프로세스 메모리 high-water mark

읽기와 달리 **쓰기는 지금 줄인다.** `create_experiment_log`는 호출 1회가 곧 트랜잭션
1회 + SELECT 2회다(`service.py`: `session.begin()` → `find_experiment` →
`find_log_by_idempotency_key`). 매 tick 이미 적재된 청크까지 다시 태우면 5초 주기·31분
실행에서 500KB짜리 컨테이너 하나만으로 대략 `62청크 × 372tick / 2 ≈ 11,500`회이고,
그중 첫 tick 이후는 전부 no-op이다. 컨테이너 8개와 동시 실행 2건을 곱하면 실험 1건당
수만 건이 된다. 읽기 증폭은 kube-apiserver 전송량이지만 이쪽은 API가 함께 쓰는
Cloud SQL 인스턴스의 트랜잭션이다.

`SeqCursor`가 `{(pod_name, container): next_seq}`를 **프로세스 메모리에만** 들고
그보다 앞선 청크를 건너뛴다. 영속 커서가 아니므로 위의 "재시작에 안전하다"는 전제가
그대로 유지된다 — 재시작하면 표가 비고, 그러면 지금까지처럼 전부 다시 계산해 같은
키·같은 내용으로 올려 멱등 no-op이 된다.

상주 프로세스라 Pod마다 항목이 쌓이면 누수이므로, tick마다 그 tick에 관측되지 않은
Pod의 항목을 버린다. 잘못 버려도 손실은 없다 — 다시 0부터 계산할 뿐이다.
...
| 그 외 `ApiException` | 이상 | `pod_log_read_failed` — 그 컨테이너 skip, 다음 컨테이너 계속 |
| `IdempotencyConflictError` | 이상 | `log_write_conflict` — **그 청크만** 버리고 다음 청크 계속 |
| 그 외 쓰기 오류 | 이상 | `log_write_failed` — **그 컨테이너의 남은 청크 중단**, 다음 tick에 재시도 |

**충돌과 일시 장애를 같은 코드로 뭉치지 않는다.** 운영 중 로그만 보고 "키 규칙 결함"과
"DB 일시 장애"를 구분할 수 있어야 하고, 대응도 다르다.

- 충돌은 재시도해도 같은 결과다(키 규칙 결함이거나 로그 회전으로 경계가 밀린 경우).
  멈추면 그 컨테이너 로그가 Pod 수명 내내 그 지점에서 끊긴다 — **구멍 하나가
  blackout보다 낫다.** 그래서 건너뛰고 커서를 그만큼 전진시켜 같은 충돌을 매 tick
  되풀이하지 않는다.
- 일시 장애는 기다리면 낫는다. 여기서 건너뛰면 저장된 커서가 없는 설계에서 그 구간이
  영영 비고 화면에는 로그가 이어진 것처럼 보이므로, 전진시키지 않고 멈춘다.

`log_type` 상한(32자) 초과로 나는 `ValidationError`도 `log_write_failed`로 떨어진다.
그 경우 그 컨테이너 로그가 매 tick 통째로 버려지고 화면에는 "그 단계 로그가 없음"으로만
보이므로, 컨테이너 이름 길이는 `build_executor_job` 산출물을 순회하는 테스트로
고정한다.
