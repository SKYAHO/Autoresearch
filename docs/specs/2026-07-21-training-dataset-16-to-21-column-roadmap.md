...
| **Phase 4a** | `user_static_feature`/`user_dynamic_feature`/`video_feature`/`training_entity` 4개 테이블을 순수 SQL 배치 job으로 실제 생성 | 대상 테이블은 Terraform이 스키마를 사전에 만들어둬야 함(`TRUNCATE TABLE`이 존재하지 않는 테이블에는 실패) | 구현 완료 (#207) — `src/pipeline/build_feature_tables.py`로 초안 구현 후, hyochangsung님의 `autoresearch/jobs/feature_store_build.py`(#242)와 중복돼 그쪽으로 통합(#245). `training_entity`만 이식, 나머지 3개는 제거 |
| **Phase 4b** | `user_topic_embedding`/`category_embedding`/`user_category_similarity` 배치 생성 + `get_historical_features()` 경유로 학습 파이프라인 전환 | Phase 3(#206) 임베딩 클라이언트를 배치로 호출하는 아키텍처, Feast(`dev` 그룹과 의존성 충돌)를 어느 이미지/그룹에서 실행할지 결정 | 임베딩 3개 테이블은 hyochangsung님의 `scripts/build_static_features.py`(#242)로 구현 완료. `get_historical_features()` 실 연결만 #214에 남아 구현 보류 |
...
1. **(Phase 1, 구현 완료)** training_dataset 16→21컬럼 확장 — DuckDB fallback 경로 (#204)
2. **(Phase 2, 구현 완료)** `preferred_category` mock 제거 — 실제 primary_categories 배선 (#205)
3. **(Phase 3, 구현 완료)** `topic_similarity` 실제 임베딩 전환 — Vertex AI text-multilingual-embedding-002 (#206)
4. **(Phase 4a, 구현 완료)** 4개 BigQuery 중간 테이블(user_static/user_dynamic/video_feature/training_entity) 배치 생성 (#207) — 이후 hyochangsung님의 `autoresearch/jobs/feature_store_build.py`(#242)와 통합(#245)
5. **(Phase 4b, 임베딩 부분 완료)** user_topic_embedding/category_embedding/user_category_similarity 배치 생성은 hyochangsung님의 `scripts/build_static_features.py`(#242)로 완료. Feast `get_historical_features()` 경유 전환만 #214에 남음
