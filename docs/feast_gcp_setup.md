...
```bash
gsutil mb -l asia-northeast3 gs://feast-staging-${GCP_PROJECT}/
```
...
# feature_store.yaml 업데이트
python scripts/setup_feast_config.py
```
...
3. `python scripts/setup_feast_config.py` 재실행
...
| `feast apply` schema error | parquet 컬럼명/타입이 FeatureView schema와 일치하는지 확인 |
