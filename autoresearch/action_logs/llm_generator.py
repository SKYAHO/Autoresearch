...
출력은 후보 순서 그대로의 위치기반 JSON 배열 하나만 허용한다. Markdown이나 주석을 넣지 마라."""
...
각 후보를 순서대로 판단해 아래 JSON만 출력하라(주석/Markdown 금지):
{{"j": [[click_propensity, watch_fraction], ...]}}

제약:
- j 는 후보 순서 그대로, 정확히 {n}개 원소를 포함한다.
- 각 원소는 [click_propensity, watch_fraction] 형태의 길이 2 배열.
...
class RuleBasedActionLogGenerator:
    ...
    def generate(self, virtual_user: dict, videos: list[dict]) -> str:
        ...
        출력은 LLM generator와 동일한 위치기반 포맷({"j": [[cp, wf], ...]})이다.
        ...
        for v in videos:
            ...
            rows.append(
                [round(propensity, 3), round(min(1.0, propensity + 0.1), 3)]
            )
