...
gen = OpenRouterActionLogGenerator(
...
- generator는 worker thread별 OpenAI client/HTTP connection pool을 재사용하며 daily runner가
  ...
  제한한다.
- provider routing mode는 `default`, `auto`, `fixed` 세 가지다. `default`는 기존
  `OPENROUTER_PROVIDER_SORT`, `OPENROUTER_ALLOW_FALLBACKS`,
  `OPENROUTER_REQUIRE_PARAMETERS` 환경값을 그대로 해석한다. `auto`는 이 환경값과
  무관하게 request의 provider payload를 완전히 생략한다. `fixed`는 정규화·검증된
  `provider_slug`가 필수이며 `provider.only=[slug]`, `allow_fallbacks=false`만
  보낸다. `auto`/`fixed`는 model 이름을 바꾸지 않는다.
- `provider_slug`는 `fixed` 이외 mode에서 금지된다. `rule_based` generator는
  non-default provider mode를 거부한다. 예를 들어 DeepInfra 단독 arm은
  `OpenRouterActionLogGenerator(provider_routing_mode="fixed",
  provider_slug="deepinfra")`로 만든다.
...
`run_daily_action_log(...)`와 `run_daily_action_log_shard(...)`는
`provider_routing_mode="default"`, `provider_slug=None`,
`expected_user_count=None`을 받는다. QA에서 `expected_user_count=100`을 주면 parquet에서
로드한 전체 user 수가 정확히 100인지 shard split 전에 검증하고, 다르면 generator나
checkpoint를 시작하지 않고 실패한다.
...
OpenRouter 요청은 공식 `X-OpenRouter-Metadata: enabled` header를 사용한다. 성공
응답에 metadata가 있으면 `openrouter_request_complete`의 `provider`를 실제 selected
provider로 기록하고 `router_attempt_count`, `router_fallback_count`,
`router_429_count`를 추가한다. metadata가 없으면 기존 response-level provider
fallback을 유지한다. `openrouter_metadata` 원문과 그 안의 summary/pipeline,
prompt/response 본문은 로그하지 않는다.
...
OpenRouter generator config에는 정규화된 provider routing mode/slug와 실제 request에
반영되는 resolved provider preferences가 포함된다. 따라서 같은 model/input이라도
`auto`와 `fixed`, 또는 서로 다른 fixed slug는 config fingerprint와 checkpoint
namespace를 공유하지 않는다.
