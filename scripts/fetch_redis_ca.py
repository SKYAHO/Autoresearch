...
[파이프라인] 피처 구간 — 셀프 호스티드 러너에서 `feast apply`를 직접 실행하는
`feast-apply.yml`(#561, 구 GKE Job #346)이 feast CLI를 띄우기 **직전**에 TLS
CA를 조달하는 구간을 담당한다.
...
이미 존재해야 하는데, feast 공식 CLI에는 CA를 조달하는 훅이 없다.
`feast-apply.yml`이 ``fetch_redis_ca.py <path> && feast apply`` 순서로
실행해 그 간극을 메운다.

[비책임] Redis 연결·IAM 인증은 ``feature_repo/redis_iam.py``가, env 주입은
