...
- 빠른 탐색 결과를 구현 또는 운영 변경의 근거로 사용하기 전에

프로젝트 구조·모듈 경계·배포 이미지·외부 계약은
`agent-project-reference.md`를 정본으로 사용합니다. 이 문서는 그 사실을
...
| Middle | 범위가 명확한 구현·버그 수정·테스트 작성, 일반 코드 또는 문서 리뷰 | Top이 정한 범위와 완료 조건 안에서 작업하고, 변경한 동작을 직접 검증합니다. |
